"""
WealthMesh demo script â€” reproduces a full private-banking conversation.
...
_CONVERSATION = [
    ...
    {
        "turn": 9,
        "message": "Send the report by email to client@wealthmesh.local â€” confirm send.",
...
async def run_demo(base_url: str, token: str = "dev-token") -> int:
    ...
    print(f"\n{BOLD}{'=' * 60}{RESET}")
    print(f"{BOLD}WealthMesh â€” Private Banking AI Demo{RESET}")
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        try:
            ...
            r.raise_for_status()
            print(f"{GREEN}âœ“ Orchestrator healthy{RESET}\n")
        except Exception as exc:
            print(f"{RED}âœ— Orchestrator not reachable: {exc}{RESET}")
        ...
        for item in _CONVERSATION:
            ...
            status = f"{GREEN}âœ“ PASS{RESET}" if ok else f"{YELLOW}~ PARTIAL{RESET}"
    ...
    if passed == total:
        ...
        print(f"{YELLOW}Most turns passed â€” LLM grounding may vary without Ollama.{RESET}")
    else:
        print(f"{RED}Several turns missed expectations â€” check if Ollama is running.{RESET}")
    print(f"{'=' * 60}{RESET}\n")
