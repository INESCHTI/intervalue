"""
Phase 1 â€” Model benchmark for RTX 2050 (4 GB VRAM, 16 GB RAM).
...
# Models to benchmark â€” (name, role, origin)
ROSTER: list[tuple[str, str, str]] = [
    ("qwen2.5:3b", "router / most agents", "ðŸ‡¨ðŸ‡³ Alibaba"),
    ("qwen2.5:7b", "financial assistant (main)", "ðŸ‡¨ðŸ‡³ Alibaba"),
    ("llama3.2:3b", "alt small agent", "ðŸ‡ºðŸ‡¸ Meta"),
    ("phi3.5", "alt small agent / tool use", "ðŸ‡ºðŸ‡¸ Microsoft"),
    ("mistral:7b-instruct", "French-origin option", "ðŸ‡«ðŸ‡· Mistral"),
    ("deepseek-r1:7b", "reasoning (on-demand only)", "ðŸ‡¨ðŸ‡³ DeepSeek"),
    ("nomic-embed-text", "embeddings (RAG)", "ðŸ‡ºðŸ‡¸ Nomic"),
]
...
def run_benchmark(quick: bool = False) -> list[ModelResult]:
    ...
    for name, role, origin in ROSTER:
        ...
        if not model_available(name):
            r.error = "not installed â€” run: ollama pull " + name
            print(f"   x {r.error}")
            results.append(r)
            continue
        ...
        print("   warming up â€¦", end=" ", flush=True)
        try:
            t0 = time.perf_counter()
        ...
        # Actual benchmark
        if not quick:
            print("   benchmarking â€¦", end=" ", flush=True)
            try:
...
def print_table(results: list[ModelResult]) -> None:
    ...
    for r in results:
        if not r.available:
            status = f"x {r.error}"
            print(f"{r.name:<28} {r.role:<30} {'â€”':>6} {'â€”':>7} {'â€”':>8}  {status}")
        elif "embed" in r.name:
            dim = r.extra.get("dim", "?")
            print(
                f"{r.name:<28} {r.role:<30} {'â€”':>6} {'â€”':>7} {r.vram_mb:>8}  "
                f"v dim={dim}  {r.total_s:.2f}s"
            )
        elif r.error:
            print(f"{r.name:<28} {r.role:<30} {'â€”':>6} {'â€”':>7} {r.vram_mb:>8}  x {r.error}")
        else:
            print(
...
def main() -> None:
    ...
    print("Agentic Mesh â€” Phase 1 Model Benchmark")
