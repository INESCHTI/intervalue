...
const DEV_USER = {
  token: 'dev-token',
  username: 'advisor@aurevia.local',
  roles: ['advisor'],
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const devSignedOut = window.sessionStorage.getItem('aurevia-dev-signed-out') === 'true'
  ...
  const login = () => {
    if (devAuth) {
      window.sessionStorage.removeItem('aurevia-dev-signed-out')
      setState(current => ({
        ...current,
        ready: true,
        authenticated: true,
  ...
  const logout = () => {
    setAuthToken(null)
    if (devAuth) {
      window.sessionStorage.setItem('aurevia-dev-signed-out', 'true')
      setState(current => ({
        ...current,
        ready: true,
        authenticated: false,
