# Aurevia Current App - Visual Architecture Plan

## Purpose

This file is the single-source visual plan for the current Aurevia application. It documents what exists today: product features, UI routes, assistant modes, voice flows, document upload/OCR, the agent mesh, local models, auth modes, deployment topology, and the verification paths that keep the system honest.
...
> [!NOTE]
> This is a current-state architecture plan, not a redesign proposal. It intentionally maps the running Aurevia app as implemented in the repo, including the recent instant/deep chat, voice-to-voice, document extraction, multilingual TTS, and Aurevia branding work.

## Product Snapshot

Aurevia is a private wealth intelligence workspace with five main web surfaces:
...
| --- | --- | --- |
| Overview | `/`, `frontend/src/pages/Dashboard.tsx` | Portfolio overview, Aurevia branding, animated logo assets, advisor workspace shell. |
...
**Aurevia local runtime and agent mesh.**
...
- `frontend/src/components/Layout.tsx` (modified): Aurevia shell, sidebar navigation, user state, sign out.
...
- **AI transparency**: the UI labels the AI assistant and warning copy says Aurevia can make mistakes.
...
- [x] Frontend loads at http://localhost:5173/ with Aurevia title and dev auth.
