"""
Eval harness â€” runs golden Q/A pairs through the agentic mesh and scores them.
...
Scoring:
  ...
  - llm_score     : LLM-as-judge (0/1) â€” only when LLM available
...
def _log_mlflow(results: list[EvalResult], run_name: str = "eval") -> None:
    ...
    if not tracking:
        print("MLFLOW_TRACKING_URI not set â€” skipping MLflow logging")
        return
    try:
        ...
        print("mlflow not installed â€” skipping")
    except Exception as exc:
