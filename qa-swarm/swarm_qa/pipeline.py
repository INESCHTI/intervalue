...
from swarm_qa.agents import evaluator, executor, fix_planner, generator, reporter
...
def run_pipeline(
    ...
    report.failure_analyses = fix_planner.analyze_report(report)
    return report
...
def main() -> None:
    ...
    print(f"Running pipeline on channels={args.channel} against {settings.sut_api_url}", flush=True)
    report = run_pipeline(channels=args.channel, limit=args.limit, sut_version=args.version)
    run_dir = reporter.save_report(report)
    print(f"\nPass rate: {report.pass_rate}% ({report.passed}/{report.total})", flush=True)
