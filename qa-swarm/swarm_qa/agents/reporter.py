...
def render_markdown(report: RunReport) -> str:
    ...
    if report.failure_analyses:
        lines += ["", "## Triage and fix plans", ""]
        for analysis in report.failure_analyses:
            ...
            lines += [
                ...
                f"- **Auto-apply:** `{plan.auto_apply}` (plan only, no code is modified)",
                "",
                "**Likely files:**",
            ]
            if plan.affected_files:
                ...
                lines.append("- No likely file identified yet.")
            ...
            for idx, step in enumerate(plan.steps, 1):
                ...
                if step.commands:
                    lines.append(f"   Commands: {', '.join(f'`{cmd}`' for cmd in step.commands)}")
            lines += ["", "**Verification:**"]
...
def render_html(report: RunReport) -> str:
    ...
    if reasons:
        ...
        fig.add_trace(go.Pie(
            labels=["All passed"], values=[1],
            marker_colors=["#22c55e"], textinfo="label",
        ), row=2, col=2)
...
def save_report(report: RunReport, out_dir: Path | str | None = None) -> Path:
    ...
    (run_dir / "fix_plan.json").write_text(
        report.model_dump_json(include={"failure_analyses"}, indent=2),
        encoding="utf-8",
    )
