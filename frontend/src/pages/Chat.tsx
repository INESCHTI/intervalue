...
function preprocessLatex(content: string): string {
  ...
  // Parenthesized expressions that contain LaTeX commands like \times \div \frac \sqrt etc.
  s = s.replace(/\(([^)]*\\(?:times|div|frac|sqrt|sum|prod|int|lim|infty|alpha|beta|gamma|delta|theta|pi|sigma|omega|cdot|leq|geq|neq|approx|pm|mp|rightarrow|leftarrow|Rightarrow|Leftarrow|forall|exists|partial|nabla|in|notin|subset|supset|cup|cap|ldots|cdots|binom|overline|underline|hat|bar|vec|dot|ddot|text)[^)]*)\)/g,
    (_, m) => `$${m}$`)
  return s
}
...
export default function Chat() {
  ...
  async function send(
    ...
    const language = settings.languageMode === 'auto' ? detectLanguage(cleanMessage) : settings.languageMode
              ...
              <div key={index} className={`message-row ${message.role === 'user' ? 'message-row-user' : ''}`}>
                ...
                {message.role === 'assistant' ? (
                  <AssistantMessage
                    ...
                    onRetry={() => {
                      if (loading) return
                      const userMsgIndex = messages.slice(0, index).map(m => m.role).lastIndexOf('user')
                      const userContent = userMsgIndex >= 0 ? messages[userMsgIndex].content : ''
                      if (!userContent) return
                      setMessages(prev => prev.filter((_, i) => i !== userMsgIndex && i !== index))
                      setTimeout(() => void send(userContent), 0)
                    }}
                  />
                ) : (
                  <div
...
function AssistantMessage({
  ...
  const showActions = !isStreaming && finalContent.trim() && !toolConfirmation

  function handleCopy() {
    void navigator.clipboard.writeText(finalContent).then(() => {
      setCopied(true)
  ...
  function handleSpeak() {
    ...
    setSpeaking(true)
    void speakLocalized(finalContent, {
    ...
    >
      ...
      {toolConfirmation ? (
        ...
        <div className="assistant-answer markdown-body">
          ...
          >
            {preprocessLatex(finalContent || (isStreaming ? 'Thinking...' : ''))}
          </Markdown>
        </div>
      )}
      {showActions && (
        <div className="message-actions">
          <button className="message-action-btn" onClick={handleCopy} title={copied ? 'Copied!' : 'Copy response'}>
            {copied ? <Check className="h-3.5 w-3.5" /> : <ClipboardCopy className="h-3.5 w-3.5" />}
            <span>{copied ? 'Copied' : 'Copy'}</span>
          </button>
          <button className="message-action-btn" onClick={onRetry} title="Retry this response">
            <RefreshCw className="h-3.5 w-3.5" />
            <span>Retry</span>
          </button>
          <button className="message-action-btn" onClick={handleSpeak} title={speaking ? 'Stop speaking' : 'Read aloud'}>
