...
export default function Voice() {
  ...
  async function startRecording() {
    ...
    if (!status?.stt.ready && Recognition) {
      ...
      recognition.lang = settings.languageMode === 'auto' ? 'en-US' : languageToLocale(settings.languageMode)
  ...
  async function answerSpokenQuestion(text: string) {
    ...
    try {
      let response = ''
      for await (const chunk of streamChat(text, conversationId.current, token)) {
  ...
  function speakInBrowser(text: string) {
    void speakLocalized(text, {
      rate: 0.96,
      pitch: 0.92,
      language: settings.languageMode === 'auto' ? undefined : settings.languageMode,
