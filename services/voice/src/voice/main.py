...
async def voice_chat(
    ...
    if not authorization:
        transcript = ""
        answer_text = "Please sign in before starting a voice conversation."
    else:
        # 1. Transcribe
        audio_bytes = await audio.read()
        transcript = _transcribe_audio(audio_bytes)
        if not transcript:
            answer_text = "I could not hear that clearly. Try again."
        # 2. Chat through orchestrator
    if authorization and transcript:
            ...
            try:
                async with client.stream(
                    ...
                    if resp.status_code == 401:
                        answer_text = "Your session is not authorized. Please sign in again."
                    else:
